...
--
-- Servidor: localhost
-- Tiempo de generación: 22-06-2015 a las 20:05:05
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vdoctores` AS select `d`.`id_doctor` AS `id_doctor`,`d`.`nombre` AS `nombre`,`d`.`numcolegiado` AS `numcolegiado`,group_concat(`c`.`id_clinica` separator ',') AS `id_clinica`,group_concat(`c`.`nombre` separator '</li><li>') AS `nombre_clinica` from ((`doctores` `d` join `clinica_doctor` `cd`) join `clinicas` `c`) where ((`d`.`id_doctor` = `cd`.`id_doctor`) and (`cd`.`id_clinica` = `c`.`id_clinica`)) group by `d`.`nombre`;
